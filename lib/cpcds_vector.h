...
#include"cppstring.h"
#define define_cpcds_vector(name,__t__)define_cpcds_vmk(name,__t__)define_cpcds_vappend(name,__t__)define_cpcds_vins(name,__t__)define_cpcds_vutil(name,__t__)static inline cppstring cpcds_vec_to_str_ptr_##name(cpcds_vector_##name*vec,cppstring(*str)(__t__)){cppstring ans=mk_empty_str();cppstring tmp;for(__t__*__it__=vec->pointer;__it__!=vec->pointer+vec->size;++__it__){tmp=(*str)(*__it__);str_combine(&ans,&tmp);cpcds_destr_str(tmp);if(__it__!=vec->pointer+vec->size-1){tmp=mk_from_cstr(", ");str_combine(&ans,&tmp);cpcds_destr_str(tmp);}}tmp=mk_from_cstr("[");str_combine(&tmp,&ans);cpcds_destr_str(ans);ans=tmp;tmp=mk_from_cstr("]");str_combine(&ans,&tmp);cpcds_destr_str(tmp);return ans;}static inline cppstring cpcds_vec_to_str_##name(cpcds_vector_##name vec,cppstring(*str)(__t__)){return cpcds_vec_to_str_ptr_##name(&vec,str);}typedef cpcds_vector_##name*cpcds_vector_##name##ptr;
