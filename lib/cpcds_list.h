...
#include"cppstring.h"
#define define_cpcds_list(name,__t__)define_cpcds_lmk(name,__t__)define_cpcds_lins(name,__t__)define_cpcds_lutil(name,__t__)define_cpcds_liter(name,__t__)inline cppstring cpcds_lis_to_str_ptr_##name(cpcds_list_##name*lis,cppstring(*str)(__t__)){cpcds_lnode_##name*node=lis->head;cppstring ans=mk_empty_str();cppstring tmp;while(node!=NULL){tmp=(*str)(node->v);str_combine(&ans,&tmp);cpcds_destr_str(tmp);if(node!=lis->tail){tmp=mk_from_cstr(", ");str_combine(&ans,&tmp);cpcds_destr_str(tmp);}node=node->next;}tmp=mk_from_cstr("[");str_combine(&tmp,&ans);cpcds_destr_str(ans);ans=tmp;tmp=mk_from_cstr("]");str_combine(&ans,&tmp);cpcds_destr_str(tmp);return ans;}inline cppstring cpcds_lis_to_str_##name(cpcds_list_##name lis,cppstring(*str)(__t__)){return cpcds_lis_to_str_ptr_##name(&lis,str);}typedef cpcds_list_##name*cpcds_list_##name##ptr;
